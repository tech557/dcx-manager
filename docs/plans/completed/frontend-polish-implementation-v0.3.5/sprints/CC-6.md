...
Status: Completed
Order: 10 | Family: `change-component` | Executor: Claude/opencode | Required skill: dcx-frontend-refactor | Required tool: Playwright/Preview
...
| Source/lock | `completed/frontend-polish-v0.3.5/output/FP-R5-synthesis.md` §CC-6 (+ FP-R5 PATCH); FP-R4 finalize spec (+ PATCH); Stage canvas + row-1/row-3 islands switch fully to light surfaces in light theme |
...
### Scope
- **In:** Consume CT-1 tokens so Stage canvas + islands switch in light theme. Depends on WM-1 + CT-1.
- **Out:** Token values (CT-1); brand identity.
...
- Light theme fully applies to stage + islands; no dark patches.

### PO Web Check (REAL pointer/drag — not `.click()`; builder cards are pointer/long-press/drag driven)
Route /builder light theme, 1440+390; expect no dark canvas/island patches (check the stage itself). Evidence → `output/evidence/CC-6-light-surfaces/`. Full route/viewport/seed/steps/expected: `completed/frontend-polish-v0.3.5/output/FP-R5-synthesis.md` §CC-6.
