...
description: Select and run proportionate frontend verification for a DCX Manager change. Use this skill when the user says "verify my change", "check if this works", "run the tests", "does this pass?", "verify the sprint", "check my PR", or when an agent has finished implementing something and needs to confirm the gates pass. Also trigger when deciding which verification tools to use for a given type of change. Do not wait for the user to specify which tools to run — apply the proportional matrix automatically.
---

<!-- Claude adapter — canonical source: agent-skills/dcx-frontend-verify/SKILL.md -->
<!-- This file adds Claude-specific MCP and hook context. -->
<!-- To update this skill, edit agent-skills/dcx-frontend-verify/SKILL.md and run: bash scripts/agent/sync-skills.sh -->
