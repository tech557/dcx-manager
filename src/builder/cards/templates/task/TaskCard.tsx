          ...
          <div className="flex flex-col items-center justify-center w-full h-full gap-1.5" id={`task-card-collapsed-${task.id}`}>
            ...
            {!resolvedDate ? (
              ...
              <div className="flex items-center gap-0.5 h-3.5 select-none font-mono" title={isLinked ? "Linked Date" : "Custom Date"}>
                ...
                <span className={`text-dcx-5xs font-bold tracking-tighter leading-none ${
          ...
          <div className="flex flex-col justify-center flex-1 min-w-0 h-full gap-0.5">
            ...
            <div className="flex items-center w-full min-w-0">
              <input
                ...
                className="bg-transparent border border-transparent hover:border-white/10 focus:border-[var(--theme-accent)]/30 hover:bg-white/[0.02] focus:bg-black/30 font-semibold text-dcx-3xs text-neutral-200 px-1 py-0 rounded outline-none transition-all w-full truncate focus:text-white"
            ...
            <div className="flex items-center justify-between w-full gap-1.5" onClick={(e) => e.stopPropagation()}>
              ...
              <div className="flex items-center gap-1 shrink-0 pr-0.5 select-none font-mono" id={`task-card-date-${task.id}`}>
                ...
                {dateDisplay ? (
                  <span className={`text-dcx-4xs font-bold tracking-tighter ${isLinked ? 'text-[var(--theme-accent)] font-extrabold' : 'text-neutral-400'}`} title={isLinked ? "Linked Date" : "Custom Date"}>
                    {dateDisplay}
                  </span>
                ) : (
